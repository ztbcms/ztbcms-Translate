    ...
    <div id="app" style="height: 100%;" v-cloak>
        <div style="
            ...
            <div style="position: relative;padding: 4px;height: 60px;width: 25%;float: left;">
                ...
                <button @click="addCatalog" class="btn btn-success" style="right: 10px;margin-top;top: 15px;position: absolute;float: right">添加文档</button>
            </div>
            <div style="position: relative;padding: 4px;height: 60px;width: 75%;float: left;">
                <template v-if="selectedItem">
                    ...
                    <button @click="delCatalog(selectedItem.id)" class="btn btn-danger" style="float: right;margin-left: 15px;margin-top: 10px;">删除</button>
                    <button @click="editCatalog(selectedItem.id)" class="btn btn-primary" style="float: right;margin-left: 15px;margin-top: 10px;">编辑</button>
                </template>
            </div>
        </div>
            ...
            new Vue({
                ...
                methods: {
                    getList: function () {
                        ...
                        $.ajax({
                            url: "{:U('Translate/Catalog/getCatalogBySort')}",
                            data: where,
                            dataType: 'json',
                            type: 'get',
                    ...
                    addCatalog: function(){
                        ...
                        layer.open({
                            type: 2,
                            title: '添加',
                            content: '{:U("Translate/Catalog/catalog")}&project_id='+that.id,
                            area: ['50%', '50%'],
                            end: function(){
                                that.getList();
                            }
                    ...
                    editCatalog: function(id){
                        ...
                        layer.open({
                            type: 2,
                            title: '修改',
                            content: '{:U("Translate/Catalog/catalog")}&id='+id+'&project_id='+that.id,
                            area: ['50%', '50%'],
                            end: function(){
                                that.getList();
                            }
                    ...
                    editContent: function(id, item){
                        var that = this;
                        that.iframe_src = '{:U("Translate/Content/index")}&catalog_id='+id;
                        this.selectedItem = item
                    },
                    delCatalog: function(id){
                        var that = this;
                        layer.confirm('是否确定删除', {title: '删除'}, function(){
                            $.ajax({
                                url: "{:U('Translate/Catalog/delCatalog')}",
                                data: {id: id},
                                dataType: 'json',
                                type: 'post',
                                success: function (res) {
